...
fn make_init_default_background() -> Vec<(u8, u8, u8, u8)> {
    ...
    for _ in 0..HEIGHT {
        for _ in 0..WIDTH {
            pixels.push((DEFAULT_BACKGROUND_COLOR.0, DEFAULT_BACKGROUND_COLOR.1, DEFAULT_BACKGROUND_COLOR.2, DEFAULT_BACKGROUND_COLOR.3));
        }
    }
    return pixels
}
...
pub fn make_init_frame(image: Option<DynamicImage>) -> Vec<(u8, u8, u8, u8)> {
    match image {
        None => {
            return make_init_default_background()
        }
        Some(image) => {
            let (width, height) = image.dimensions();
            if width < WIDTH || height < HEIGHT {
                eprintln!("Error: background image is smaller than screensize; Initialized with default background");
                return make_init_default_background()
            }
            else {
                let mut pixels = Vec::with_capacity((WIDTH*HEIGHT) as usize);
